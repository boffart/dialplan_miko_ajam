...
class AGI_AsteriskManager {
    ...
    function ping_ami_listner($pingname = 'CdrConnector'){
        ...
        $params   = array('Operation'=>'Add', 'Filter' => 'Event: UserEvent');
    ...
    private function wait_response_get_sub_data(&$parameters, $end_string = '', $event_as_array=true) {
    ...
    function IAXpeerlist(){
        ...
        $arr_peer = (isset($data['PeerEntry']) && is_array($data['PeerEntry']))?$data['PeerEntry']:array();
        return $arr_peer;
    }
    ...
    function IAXregistry(){
        ...
        $data = (isset($result['data']) && is_array($result['data']))?$result['data']:array();
        $arr_peer = (isset($data['RegistryEntry']) && is_array($data['RegistryEntry']))?$data['RegistryEntry']:array();
        return $arr_peer;
    }
     ...
     * @return array
     */
    public function MeetmeList($Conference, $ActionID='') {
        if(empty($ActionID)){
     ...
     * @return array
     */
    public function MeetmeListRooms($ActionID='') {
        if(empty($ActionID)){
            $ActionID = Util::generateRandomString(5);
        }
        $parameters = array(
